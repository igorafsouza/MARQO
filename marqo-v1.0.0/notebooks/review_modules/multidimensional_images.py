...
class MultTiffStitcher:
    ...
    def _set_parameters(self, sample_path, user_id):
        ...
        try:
            ...
            self.multitiff_dir = config_data['directories']['tiff']
            ...
            self.multitiff_dir = config_data['directories']['product_directories']['tiff']
    ...
    def _create_figure(self, roi_img, x, y, scale_factor=1.0):
        ...
        fig.add_trace(
            go.Scatter(
                x=np.trunc(x * scale_factor),
                y=np.trunc(y * scale_factor),
                mode="markers",
                marker_opacity=0
            )
        )
    ...
    def display_tilemap(self):
        def handle_click(trace, points, state):
            centroid = f'{points.xs[0]}, {points.ys[0]}'
        ...
        x = np.trunc(x * scale_factor)
        y = np.trunc(y * scale_factor)    
        tilestats_map = {f'{int(coord[0])}, {int(coord[1])}': roi_index for roi_index, coord in enumerate(zip(x, y))}
    ...
    def stitch(self, norm_coords):

        max_x, max_y = self._get_max(norm_coords)
        canvas = np.zeros((len(self.markers), max_y + 1000, max_x + 1000))

        all_cells = []
        for roi in self.selected_rois:
            roi_img_path = os.path.join(self.multitiff_dir, f'{self.sample_name}_multichanneloutput_ROI#{roi}.ome.tif')
            roi_geojson_path = os.path.join(self.geojson_dir, f'{self.sample_name}_ROI#{roi}_{self.marker_tile}.json')
            x, y = norm_coords[roi]
    ...
    def execute(self, **kwargs):
        ...
        canvas, cells = self.stitch(norm_coords)
